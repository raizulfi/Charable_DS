<div class="content py-3">
    <div class="card card-outline card-primary rounded-0 shadow">
        <div class="card-header">
            <h5 class="card-title"><b>My Donations</b></h5>
        </div>
        <div class="card-body">
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    <?php 
                    $i = 1;
                    $donation_history = $conn->query("SELECT d.*, p.name FROM `donation` d inner join program p on d.program_id = p.id 
                    where donor_id = '{$_settings->userdata('id')}' 
                    order by `status` and `name` asc,unix_timestamp(date_donated) desc ");

                    while($row = $donation_history->fetch_assoc()):
                    ?>
                    <tr>
                            ...
                            <?php 
                                switch($row['status']){
                                    case 0:
                                        ...
                                        break;
                                    case 1:
                                        echo '<span class="badge badge-primary bg-gradient-success px-3 rounded-pill">Completed</span>';
                                        break;
                                    /*case 2:
...
<script>
    $(function(){
        $('.view_data').click(function(){
            uni_modal("View My Donation Details - <b>"+($(this).attr('data-code'))+"</b>","donation_history/view_donation.php?id="+$(this).attr('data-id'),'mid-large')
        })
        $('table').dataTable();
    })
</script>