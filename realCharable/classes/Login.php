...
class Login extends DBConnection {
	...
	public function login(){
		...
		$stmt = $this->conn->prepare("SELECT * from administrator where email = ? and `password` = ? ");
		...
		return json_encode(array('status'=>'incorrect','last_qry'=>"SELECT * from administrator where email = '$username' and `password` = md5('$password') "));
		}
	}
	...
	public function login_client(){
		extract($_POST);
		$password = md5($password);
		$stmt = $this->conn->prepare("SELECT * from donor where email = ? and `password` =? and delete_flag = ?  ");
		$delete_flag = 0;
