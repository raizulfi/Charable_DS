...
Class Users extends DBConnection {
	...
	public function save_users(){
		extract($_POST);
		$data = '';
		$chk = $this->conn->query("SELECT * FROM `administrator` where email ='{$email}' ".($id>0 ? " and id!= '{$id}' " : ""))->num_rows;
		if($chk > 0){
			return 3;
			exit;
		}
		...
		if(empty($id)){
			$qry = $this->conn->query("INSERT INTO administrator set {$data}");
			if($qry){
					...
					if(!in_array($type,$allowed)){
						...
						if($gdImg){
								...
								if(isset($uploaded_img) && $uploaded_img == true){
									$qry = $this->conn->query("UPDATE administrator set avatar = concat('{$fname}','?v=',unix_timestamp(CURRENT_TIMESTAMP)) where id = '$id' ");
								}
						}else{
			...
			$qry = $this->conn->query("UPDATE administrator set $data where id = {$id}");
			if($qry){
				...
				return "UPDATE administrator set $data where id = {$id}";
			}
	...
	public function delete_users(){
		extract($_POST);
		$avatar = $this->conn->query("SELECT avatar FROM administrator where id = '{$id}'")->fetch_array()['avatar'];
		$qry = $this->conn->query("DELETE FROM administrator where id = $id");
		if($qry){
	...
	public function save_client(){
		...
		if(empty($_POST['id'])){
			...
			$code = sprintf("%'.05d",1);
			while(true){
				$check = $this->conn->query("SELECT * FROM `donor` where code = '{$prefix}{$code}'")->num_rows;
				if($check > 0){
					$code = sprintf("%'.05d",ceil($code) + 1);
				}else{
					break;
		...
		extract($_POST);
		if(isset($oldpassword) && !empty($id)){
			$current_pass = $this->conn->query("SELECT * FROM `donor` where id = '{$id}'")->fetch_array()['password'];
			if(md5($oldpassword) != $current_pass){
				$resp['status'] = 'failed';
				$resp['msg'] = ' Incorrect Current Password';
				return json_encode($resp);
		...
		$check  = $this->conn->query("SELECT * FROM `donor` where email = '{$email}' and delete_flag = 0 ".(!empty($id) ? " and id !='{$id}'" : ''))->num_rows;
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = " Email already exists";
		}else{
			if(empty($id)){
				$sql = "INSERT INTO `donor` set {$data}";
			}else{
				$sql = "UPDATE `donor` set {$data} where id = '{$id}'";
			}
			$save = $this->conn->query($sql);
			if($save){
				$resp['status'] = "success";
				...
				if(empty($id)){
					if(strpos($_SERVER['HTTP_REFERER'], 'client/register.php') > -1){
						...
						$resp['msg'] = " Donor's Account has been registered successfully.";
					}
				}else{
					if($this->settings->userdata('login_type') == 3){
						$resp['msg'] = " Your account details has been updated successfully.";
					}else{
						$resp['msg'] = " Donor's Account Details has been updated successfully.";
					}	
				}
				...
				if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
					...
					if(!in_array($type,$allowed)){
						...
						if($gdImg){
								...
								imagedestroy($t_image);
								if($uploaded_img){
									$qry = $this->conn->query("UPDATE `donor` set avatar = concat('{$fname}','?v=',unix_timestamp(CURRENT_TIMESTAMP)) where id = '$vid' ");
									if($this->settings->userdata('id') == $id && $this->settings->userdata('login_type') == 2)
										$this->settings->set_userdata('avatar',$fname."?v=".(time()));
								}
						}else{
	...
	public function delete_client(){
		extract($_POST);
		$qry = $this->conn->query("UPDATE donor set delete_flag = 1 where id = $id");
		if($qry){
